    ...
    async def when_digivolving_effect(self, ws):
        ...
        for c in self.bot.game['player2Digi'][digimon_index][:-1]:
            if c['name'] == 'Beelzemon' or c['name'] == 'X Antibody':
                ...
                await self.bot.send_message(ws, f"Beelzemon (X Antibody) [When Digivolving] effect: Cards in trash: {len(self.bot.game['player2Trash'])}, {n_sec_trash} card/s get trashed from your security stack. Resolve the effect and press ok to continue.")
