...
class BT12_085_Beelzemon_X_Antibody(Card):
    ...
    async def when_digivolving_effect(self, ws):
        ...
        for c in self.bot.game['player2Digi'][digimon_index][:-1]:
            if c['name'] == 'Beelzemon' or c['name'] == 'X Antibody':
                n_sec_trash = min(math.floor(len(self.bot.game['player2Trash'])/10), len(self.bot.game['player1Security']))
                await self.bot.send_message(ws, f"Beelzemon (X Antibody) [When Digivolving] effect: Cards in trash: {len(self.bot.game['player2Trash'])}, {n_sec_trash} card/s get trashed from your security stack. Resolve the effect and type ok in chat to continue.")
