...
export default function ChooseMainDeckSleeve({ deckId, selectedSleeve, setSelectedSleeve }: ChooseCardSleeveProps) {
        ...
        <GridContainer>
            {sleeves.map((sleeve) => (
                <Tooltip
                    ...
                    key={sleeve.name}
                    PopperProps={{
                        modifiers: [{ name: "offset", options: { offset: [-2, 10] } }],
                    }}
                    slotProps={{
                        tooltip: {
                            sx: {
                ...
                >
                    <SleeveButton
                        key={sleeve.name}
                        alt={sleeve.name}
                        src={getSleeve("Digimon", sleeve.name)}
                        chosen={selectedSleeve === sleeve.name}
                        onClick={() => {
                            playButtonClickSfx();
                            axios
                                .put(`/api/profile/decks/${deckId}/sleeve/${sleeve.name}`)
...
const GridContainer = styled.div`
    ...
    grid-template-rows: repeat(11, 84px);
    gap: 2px;
    position: relative;
    padding: 3px;
    margin: 8px;
