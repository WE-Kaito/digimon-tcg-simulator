...
export default function ModifierMenu({ sendSetModifiers }: ModifierMenuProps) {
    ...
    const haveModifiersChanged =
        colors?.length &&
        (card?.modifiers.plusDp !== plusDp ||
                ...
                <StyledFieldset>
                    ...
                    <Stack alignItems={"flex-start"} gap={"5px"}>
                        <NumericStack>
                            ...
                            <IconButton onClick={handleSubDp} color={"error"}>
                                <RemoveCircleOutlined />
                            </IconButton>
                            <ValueSpan>{dpValue}</ValueSpan>
                            <IconButton onClick={handleAddDp} color={"success"}>
                                <AddCircleOutlined />
                            </IconButton>
                        </NumericStack>
                        <NumericStack>
                            <ValueLabelSpan>Security Attack</ValueLabelSpan>
                            <IconButton onClick={handleSubSec} color={"error"}>
                                <RemoveCircleOutlined />
                            </IconButton>
                            <ValueSpan>{secAttackValue}</ValueSpan>
                            <IconButton onClick={handleAddSec} color={"success"}>
                                <AddCircleOutlined />
                            </IconButton>
                        </NumericStack>
                                        ...
                                        (keyword) =>
                                            !keywords.includes(keyword) && (
                                                <Item
                                                    ...
                                                    key={`${keyword}_selection`}
                                                    style={{
                                                        ...(keyword === "SICK" && {
                                                            background: "rgba(1,78,114,0.9)",
                                                            borderRadius: 3,
                                                        }),
                                                    }}
