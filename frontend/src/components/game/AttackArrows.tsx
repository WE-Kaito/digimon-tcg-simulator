...
export default function AttackArrows() {
    const arrowFrom = useGameUIStates((state) => state.arrowFrom);
    const arrowTo = useGameUIStates((state) => state.arrowTo);
...
const StyledArrow = styled(Arrow)<{ isFromOpponent: boolean; isEffect: boolean }>`
    position: relative;
    pointer-events: none;
    stroke: ${({ isFromOpponent, isEffect }) =>
        isFromOpponent ? (isEffect ? "#ECAA4D" : "crimson") : isEffect ? "aquamarine" : "#007fff"};
    fill: ${({ isFromOpponent, isEffect }) =>
        isFromOpponent ? (isEffect ? "#ECAA4D" : "crimson") : isEffect ? "aquamarine" : "#007fff"};
